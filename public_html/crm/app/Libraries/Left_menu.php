...
class Left_menu {
    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            // SIPRI baseline scope:
            // - module_sipri enables the SIPRI section
            // - sipri_mode hides most CRM items to avoid confusing users during SIPRI rollout
            $module_sipri = get_setting("module_sipri");
            $sipri_mode = get_setting("sipri_mode");

            if ($module_sipri && ($this->ci->login_user->is_admin || get_array_value($permissions, "sipri_access"))) {
                //group menu (root item + submenus)
                $sidebar_menu["sipri"] = array(
                    "name" => "sipri",
                    "url" => "sipri",
                    "class" => "layers",
                    "submenu" => array(
                        array("name" => "sipri_ideas", "url" => "sipri/ideas", "class" => "file-text"),
                        array("name" => "sipri_approvals", "url" => "sipri/approvals", "class" => "check-circle"),
                        array("name" => "sipri_contacts", "url" => "sipri/contacts", "class" => "users"),
                        array("name" => "sipri_documents", "url" => "sipri/documents", "class" => "book-open"),
                    )
                );
            }

            if ($sipri_mode) {
                //Keep only minimal items alongside SIPRI (dashboard + messages + settings)
                //Messages module should remain available for communications during transition.
                if (get_setting("module_message")) {
                    $sidebar_menu["messages"] = array("name" => "messages", "url" => "messages", "class" => "message-square");
                }

                //Settings is auto-added on save, but include it for default menu.
                if ($this->ci->login_user->is_admin || get_array_value($permissions, "can_manage_all_kinds_of_settings")) {
                    $sidebar_menu["settings"] = array("name" => "settings", "url" => "settings", "class" => "settings");
                }

                return array_values($sidebar_menu);
            }
