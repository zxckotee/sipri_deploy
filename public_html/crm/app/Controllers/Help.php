...
class Help extends Security_Controller {
    ...
    //SIPRI knowledge base articles list (managed by SIPRI managers)
    function sipri_knowledge_base_articles() {
        if (!($this->login_user->is_admin || get_array_value($this->login_user->permissions, "sipri_manage"))) {
            app_redirect("forbidden");
        }

        $view_data["type"] = "sipri_knowledge_base";
        return $this->template->rander("help_and_knowledge_base/articles/index", $view_data);
    }
    ...
    //SIPRI knowledge base categories list (managed by SIPRI managers)
    function sipri_knowledge_base_categories() {
        if (!($this->login_user->is_admin || get_array_value($this->login_user->permissions, "sipri_manage"))) {
            app_redirect("forbidden");
        }
    ...
    function save_category() {
        ...
        // SIPRI knowledge base: bind to tenant/department
        if ($this->request->getPost('type') === "sipri_knowledge_base") {
            if (!($this->login_user->is_admin || get_array_value($this->login_user->permissions, "sipri_manage"))) {
                app_redirect("forbidden");
            }
