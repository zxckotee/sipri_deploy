...
class LeftNavigationIcon extends DBClass
{
  private $table = "left_navigation_icon";
  ...
  function getAll()
  {
    ...
    while ($s = $result->fetch_array(MYSQLI_ASSOC)) {
      $m["icon_url"] = "{$path}images/left_navigation_icon/{$s['icon']}";
      $res[] = $s;
    }
  ...
  function getAllEnable()
  {
    ...
    $arr["icon_url"] = "{$path}images/left_navigation_icon/{$arr['icon']}"; 
  ...
  function insert()
  {
    ...
    if ($erreur == "") {
      ...
      if (isset($this->icon) && substr($_FILES['image']['type'], 0, 5) == "image") {
        ...
        move_uploaded_file($_FILES['image']['tmp_name'], "../images/left_navigation_icon/menu_item_" . $id . ".png");
      }
  ...
  function update()
  {
    ...
    if ($erreur == "") {
      ...
      if (isset($this->icon) && substr($_FILES['image']['type'], 0, 5) == "image") {
        move_uploaded_file($_FILES['image']['tmp_name'], "../images/left_navigation_icon/menu_item_" . $this->id . ".png");
      }
