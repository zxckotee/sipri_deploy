...
class Notifications_model extends Crud_model {
    ...
    function create_notification($event, $user_id, $options = array()) {
        ...
        // SIPRI tables
        $sipri_ideas_table = $this->db->prefixTable('sipri_ideas');
        $sipri_idea_approvals_table = $this->db->prefixTable('sipri_idea_approvals');
        ...
        $sipri_idea_id = get_array_value($options, "sipri_idea_id");
        ...
        // SIPRI: idea author
        if (in_array("sipri_idea_author", $notify_to_terms) && $sipri_idea_id) {
            $where .= " OR ($users_table.id=(SELECT $sipri_ideas_table.created_by FROM $sipri_ideas_table WHERE $sipri_ideas_table.id=$sipri_idea_id)) ";
        }

        // SIPRI: idea approvers
        if (in_array("sipri_idea_approvers", $notify_to_terms) && $sipri_idea_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($sipri_idea_approvals_table.approver_user_id) FROM $sipri_idea_approvals_table WHERE $sipri_idea_approvals_table.idea_id=$sipri_idea_id)) ";
        }
